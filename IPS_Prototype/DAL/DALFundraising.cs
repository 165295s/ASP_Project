...
namespace IPS_Prototype.DAL
{
    public class DALFundraising
    {
        ...
        public DataTable getLatestDate()
        {
            //            string queryStr = @"SELECT A.* 
            //FROM TBL_DONATION as A 
            //LEFT JOIN TBL_DONATION as B on A.DONATION_ID = B.DONATION_ID
            //ORDER BY A.DONATION_DT desc";
            string queryStr = @"SELECT A.*, p.FULLNAME_NAMETAGS, o.NAME AS FULLNAME_NAMETAGS, , ISNULL(AVG(Indicator),0) as Indicator 
            FROM TBL_DONATION as A
LEFT JOIN membership.TBL_PERSON p on A.PERSON_ID = p.PERSON_ID
LEFT JOIN membership.TBL_ORGANISATION o on A.ORG_ID = o.ORG_ID
LEFT JOIN TBL_DONATION as B on A.DONATION_ID = B.DONATION_ID
ORDER BY A.DONATION_DT desc";
