...
where p.PERSON_ID =" + personId + ";";
 ...
 //               @"SELECT distinct o.ORG_ID, o.NAME, m.DONOR_TIER, m.EXPIRY_DT 
 //FROM membership.TBL_ORGANISATION o INNER JOIN membership.TBL_MEMBERSHIP m ON o.ORG_ID = m.ORG_ID";

            //For the new Membership Renewal page (Qiqi)
            string queryStr = @"SELECT distinct o.ORG_ID, o.NAME, m.DONOR_TIER, m.EXPIRY_DT, c.CONTRIBUTION_STATUS, c.Contribution_Id 
            FROM membership.TBL_ORGANISATION o INNER JOIN membership.TBL_MEMBERSHIP m ON o.ORG_ID = m.ORG_ID
            LEFT JOIN shared.Tbl_Contribution c on m.MEMBER_ID = c.MEMBER_ID order by c.Contribution_Id desc";

            //original hehe
            //SELECT distinct p.PERSON_ID, CONCAT(p.FIRST_NAME, ' ', p.SURNAME) AS FULLNAME, p.EMAIL_ADDR, m.DONOR_TIER,
            ...
            if (dt != null && dt.Rows.Count>0)
            {
                ...
                {
                    if (!dictionary.ContainsKey(Convert.ToInt32(row[0])))
                    {
                        dictionary.Add(Convert.ToInt32(row[0]), row);
        ...
        public DataTable GetOrganizationMembershipContribution(string organizationId)
        {
            //For the new Membership Renewal page (Qiqi)
            string queryStr = string.Format(@"SELECT distinct o.ORG_ID, o.NAME, m.DONOR_TIER, m.EXPIRY_DT, c.TER_SENT_DT, c.TER_RECEIVED_DT, c.TER_DETAILS, c.TOTAL_AMOUNT, c.CONTRIBUTION_DT, c.CONTRIBUTION_STATUS, Sum(ct.Amount) as Amount_Paid, Count(ct.Amount) As Times_Paid, c.Contribution_Id 
FROM membership.TBL_ORGANISATION o INNER JOIN membership.TBL_MEMBERSHIP m ON o.ORG_ID = m.ORG_ID
LEFT JOIN shared.Tbl_Contribution c on m.MEMBER_ID= c.MEMBER_ID
LEFT JOIN shared.TBL_CONTRIBUTION_TRANSACTION ct on ct.CONTRIBUTION_ID =c.CONTRIBUTION_ID Where
o.ORG_ID={0} Group by o.ORG_ID, c.TOTAL_AMOUNT,
o.NAME, m.DONOR_TIER, m.EXPIRY_DT, c.CONTRIBUTION_STATUS, c.CONTRIBUTION_DT, c.Contribution_Id, c.TER_SENT_DT, c.TER_RECEIVED_DT, c.TER_DETAILS", organizationId);
            DataTable dt = dbhelp.ExecDataReader(queryStr);
            return dt;
        }

        public DataTable GetIndividualMembershipContribution(string individualId)
        {
            //For the new Membership Renewal page (Qiqi)
            string queryStr = string.Format(@"SELECT distinct p.PERSON_ID, p.EMAIL_ADDR, m.DONOR_TIER, m.EXPIRY_DT, c.TER_SENT_DT, c.TER_RECEIVED_DT, c.TER_DETAILS, c.TOTAL_AMOUNT, c.CONTRIBUTION_DT, c.CONTRIBUTION_STATUS, Sum(ct.Amount) as Amount_Paid, Count(ct.Amount) As Times_Paid, c.Contribution_Id 
FROM membership.TBL_PERSON p INNER JOIN membership.TBL_MEMBERSHIP m ON p.PERSON_ID = m.PERSON_ID
LEFT JOIN shared.Tbl_Contribution c on m.MEMBER_ID= c.MEMBER_ID
LEFT JOIN shared.TBL_CONTRIBUTION_TRANSACTION ct on ct.CONTRIBUTION_ID =c.CONTRIBUTION_ID Where
p.PERSON_ID={0} Group by p.PERSON_ID, c.TOTAL_AMOUNT,
p.EMAIL_ADDR, m.DONOR_TIER, m.EXPIRY_DT, c.CONTRIBUTION_STATUS, c.CONTRIBUTION_DT, c.Contribution_Id, c.TER_SENT_DT, c.TER_RECEIVED_DT, c.TER_DETAILS", individualId);
            DataTable dt = dbhelp.ExecDataReader(queryStr);
            return dt;
        }

        public DataTable GetContributionTransaction(string contributionId)
        {
            //For the new Membership Renewal page (Qiqi)
            string queryStr = string.Format("SELECT * from shared.TBL_CONTRIBUTION_TRANSACTION Where Contribution_Id={0}", contributionId);
            DataTable dt = dbhelp.ExecDataReader(queryStr);
            return dt;
        }
        ...
        public DataTable getAllMembershipRenewalDetailPerson()
        {
            ...
            string queryStr = @"SELECT distinct p.PERSON_ID, CONCAT(p.FIRST_NAME, ' ', p.SURNAME) AS FULLNAME, p.EMAIL_ADDR, m.DONOR_TIER, m.EXPIRY_DT, c.CONTRIBUTION_STATUS, c.Contribution_Id 
            FROM membership.TBL_PERSON p INNER JOIN membership.TBL_MEMBERSHIP m ON p.PERSON_ID = m.PERSON_ID
            LEFT JOIN shared.Tbl_Contribution c on m.MEMBER_ID = c.MEMBER_ID order by c.Contribution_Id desc";
        ...
        public void UpdatePaymentStatusOrganisation(IndividualContribution individualContribution)
        {
            List<SqlCommand> transcommand = new List<SqlCommand>();
            SqlCommand mycmd = new SqlCommand();
            try
        ...
        on c.CONTRIBUTION_ID = ct.CONTRIBUTION_ID where ct.Contribution_Id={1} and ct.Member_Id = (select Member_Id from membership.TBL_Membership
          where ORG_ID = {0}) group by ct.TOTAL_AMOUNT, ct.CONTRIBUTION_ID order by ct.CONTRIBUTION_ID desc", individualContribution.OrgId, individualContribution.ContributionId);
                ...
                string newQuery = string.Format("update shared.TBL_CONTRIBUTION set CONTRIBUTION_STATUS ='{0}', Contribution_Paid=Contribution_Paid+{1} Where Member_Id=(select Member_Id from membership.TBL_Membership " +
                    " where ORG_ID={3}) and Contribution_Id={2}", individualContribution.Status, individualContribution.Amoount, individualContribution.ContributionId, individualContribution.OrgId);
        ...
        public int EditTerIndividual(string email, DateTime sentdate, DateTime receiveddate, string details, string contributionId)
        {
            int result = 0;
            List<SqlCommand> transcommand = new List<SqlCommand>();
            SqlCommand mycmd = new SqlCommand();
            ...
            string commandtext = @"UPDATE c SET c.TER_SENT_DT = CONVERT(date,@tersentdate,103), c.TER_RECEIVED_DT = CONVERT(date,@terreceiveddate,103), c.TER_DETAILS = @terdetails 
            FROM shared.TBL_CONTRIBUTION c INNER JOIN membership.TBL_MEMBERSHIP m
                ON m.MEMBER_ID = c.MEMBER_ID INNER JOIN membership.TBL_PERSON p
                ON p.PERSON_ID = m.PERSON_ID
                WHERE p.EMAIL_ADDR = @EMAIL_ADDR1 and c.CONTRIBUTION_ID = @contributionID";
            mycmd = dbhelp.CreateCommand(commandtext, CommandType.Text, "@tersentdate", sentdate, "@terreceiveddate", receiveddate, "@terdetails", details, "@EMAIL_ADDR", email, "@EMAIL_ADDR1", email, "@contributionID", contributionId);
        ...
        public int EditTerOrganisation(string name, DateTime sentdate, DateTime receiveddate, string details, string contributionId)
        {
            int result = 0;
            ...
            //    ON m.MEMBER_ID = c.MEMBER_ID INNER JOIN membership.TBL_ORGANISATION o
            //    ON o.ORG_ID = m.ORG_ID
            //    WHERE o.NAME = @NAME1 and ct.CONTRIBUTION_ID = @contributionID";
            string commandtext = @"UPDATE c SET c.TER_SENT_DT = CONVERT(date,@tersentdate,103), c.TER_RECEIVED_DT = CONVERT(date,@terreceiveddate,103), c.TER_DETAILS = @terdetails 
            FROM shared.TBL_CONTRIBUTION c INNER JOIN membership.TBL_MEMBERSHIP m
                ON m.MEMBER_ID = c.MEMBER_ID INNER JOIN membership.TBL_ORGANISATION o
                ON o.ORG_ID = m.ORG_ID
                WHERE o.NAME = @NAME1 and c.CONTRIBUTION_ID = @contributionID";
            mycmd = dbhelp.CreateCommand(commandtext, CommandType.Text, "@tersentdate", sentdate, "@terreceiveddate", receiveddate, "@terdetails", details, "@NAME", name, "@NAME1", name, "@contributionID", contributionId);
            transcommand.Add(mycmd);
            result = dbhelp.ExecTrans(transcommand);
            return result;
        }
        ...
        public int EditOrgInstallment(string donortier, string expirydate, string name, DateTime modified, string contriid)
        {
            int result = 0;
            List<SqlCommand> transcommand = new List<SqlCommand>();
            SqlCommand mycmd = new SqlCommand();
            ...
            string commandtext = @"UPDATE m SET m.DONOR_TIER = @donor_tier, m.EXPIRY_DT = @expiry_date, m.MODIFIED_DT = CONVERT(date, @modified_date, 103) FROM membership.TBL_MEMBERSHIP m
                inner join membership.TBL_ORGANISATION o ON m.ORG_ID = o.ORG_ID inner join shared.TBL_CONTRIBUTION c ON c.MEMBER_ID = m.MEMBER_ID
                inner join shared.TBL_CONTRIBUTION_TRANSACTION ct on c.CONTRIBUTION_ID = ct.CONTRIBUTION_ID WHERE o.NAME = @NAME1 and c.CONTRIBUTION_ID = @contriId";
            mycmd = dbhelp.CreateCommand(commandtext, CommandType.Text, "@donor_tier", donortier, "@expiry_date", expirydate, "@NAME", name, "@modified_date", modified, "@NAME1", name, "@contriId", contriid);
